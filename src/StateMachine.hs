...
transition game s r = case (stateValue s, stateData s, r) of
    (v, GameDatum bs Nothing, Play c)
        | lovelaces v == gStake game         -> Just ( Constraints.mustBeSignedBy (gSecond game)                    <>
                                                       Constraints.mustValidateInTimeRange (Interval.lessThan $ gPlayDeadline game - 1)
    ...
    (v, GameDatum _ (Just _), Reveal _)
        | lovelaces v == (2 * gStake game)   -> Just ( Constraints.mustBeSignedBy (gFirst game)                     <>
                                                       Constraints.mustValidateInTimeRange (Interval.lessThan $ gRevealDeadline game - 1)
                                                     , State Finished mempty
                                                     )
    (v, GameDatum _ Nothing, ClaimFirst)
        | lovelaces v == gStake game         -> Just ( Constraints.mustBeSignedBy (gFirst game)                     <>
                                                       Constraints.mustValidateInTimeRange (Interval.from $ 1 + gPlayDeadline game)
                                                     , State Finished mempty
                                                     )
    (v, GameDatum _ (Just _), ClaimSecond)
        | lovelaces v == (2 * gStake game)   -> Just ( Constraints.mustBeSignedBy (gSecond game)                    <>
                                                       Constraints.mustValidateInTimeRange (Interval.from $ 1 + gRevealDeadline game)
                                                     , State Finished mempty
                                                     )
    _                                        -> Nothing
...
firstGame fp = do
    ...
    case m of
        ...
        Just (o, _) -> case Plutus.Script.Utils.V2.Typed.Scripts.tyTxOutData $ Plutus.Script.Utils.V2.Typed.Scripts.tyTxOutRefOut $ ocsTxOutRef o of
